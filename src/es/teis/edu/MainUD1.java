...
public class MainUD1 {
    ...
    private static void escribirIdPartidosCSV(ArrayList<Partido> partidos) throws FileNotFoundException {
        IPersistencia csvPersistencia = new CSVPersistencia();
        csvPersistencia.escribir(partidos, ELECCIONES_OUTPUT_FILE_CSV);
        File archivo = new File(ELECCIONES_OUTPUT_FILE_CSV);
        if (archivo.exists()){
            File archivoBK = new File(ELECCIONES_OUTPUT_FILE_CSV_BK);
